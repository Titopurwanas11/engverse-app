...
import { ScoreProgressionChart, initializeScoreChart } from './chart';
...
export default class DashboardPage {
    render() {
        console.log('DashboardPage: render method called. Returning HTML string.');
        return `
    ...
    afterRender() {
        console.log('DashboardPage: afterRender method called. Attempting to initialize chart.');
        setTimeout(() => {
            const ctx = document.getElementById('scoreChart');
            console.log('DashboardPage: Canvas context (ctx):', ctx);
            if (ctx) {
                initializeScoreChart();
                console.log('DashboardPage: Chart initialization successful.');
            } else {
                console.error('DashboardPage: Error: Canvas element with ID "scoreChart" not found in DOM!');
            }
        }, 0);
    }
}
